...
static void exit_with_usage(void) {
    ...
    fprintf(stderr, "    or\n       %s -r [-s [-]offset] [-c cols]", pname);
    fprintf(stderr, " [-ps] [infile [outfile]]\n");
    fprintf(stderr, "Options:\n");
    fprintf(stderr, "    -a          toggle autoskip: A single '*'");
    fprintf(stderr, "replaces nul-lines. Default off.\n");
    fprintf(stderr, "    -b          binary digit dump ");
    fprintf(stderr, "(incompatible with -ps,-i,-r). Default hex.\n");
    fprintf(stderr, "    -C          capitalize variable names in C ");
    fprintf(stderr, "include file style (-i).\n");
    fprintf(stderr, "    -c cols     format <cols> octets per line.");
    fprintf(stderr, " Default 16 (-i: 12, -ps: 30).\n");
    fprintf(stderr, "    -E          show characters in EBCDIC. Default ASCII.\n");
    fprintf(stderr, "    -e          little-endian dump (incompatible with -ps,-i,-r).\n");
    fprintf(stderr, "    -g          number of octets per group ");
    fprintf(stderr, "in normal output. Default 2 (-e: 4).\n");
    fprintf(stderr, "    -h          print this summary.\n");
    fprintf(stderr, "    -i          output in C include file style.\n");
    fprintf(stderr, "    -l len      stop after <len> octets.\n");
    fprintf(stderr, "    -o off      add <off> to the displayed file position.\n");
    fprintf(stderr, "    -ps         output in postscript plain hexdump style.\n");
    fprintf(stderr, "    -r          reverse operation: convert");
    fprintf(stderr, " (or patch) hexdump into binary.\n");
    fprintf(stderr, "    -r -s off   revert with <off> added to file");
    fprintf(stderr, " positions found in hexdump.\n");
 ...
 */
static int huntype(FILE *fpi, FILE *fpo, FILE *fperr, int cols, int hextype, long base_off) {
...
static unsigned char etoa64[] = {0040, 0240, 0241, 0242, 0243, 0244, 0245, 0246, 0247, 0250, 0325, 0056, 0074, 0050,
    0053, 0174, 0046, 0251, 0252, 0253, 0254, 0255, 0256, 0257, 0260, 0261, 0041, 0044, 0052, 0051, 0073, 0176, 0055,
    0057, 0262, 0263, 0264, 0265, 0266, 0267, 0270, 0271, 0313, 0054, 0045, 0137, 0076, 0077, 0272, 0273, 0274, 0275,
    0276, 0277, 0300, 0301, 0302, 0140, 0072, 0043, 0100, 0047, 0075, 0042, 0303, 0141, 0142, 0143, 0144, 0145, 0146,
    0147, 0150, 0151, 0304, 0305, 0306, 0307, 0310, 0311, 0312, 0152, 0153, 0154, 0155, 0156, 0157, 0160, 0161, 0162,
    0136, 0314, 0315, 0316, 0317, 0320, 0321, 0345, 0163, 0164, 0165, 0166, 0167, 0170, 0171, 0172, 0322, 0323, 0324,
    0133, 0326, 0327, 0330, 0331, 0332, 0333, 0334, 0335, 0336, 0337, 0340, 0341, 0342, 0343, 0344, 0135, 0346, 0347,
    0173, 0101, 0102, 0103, 0104, 0105, 0106, 0107, 0110, 0111, 0350, 0351, 0352, 0353, 0354, 0355, 0175, 0112, 0113,
    0114, 0115, 0116, 0117, 0120, 0121, 0122, 0356, 0357, 0360, 0361, 0362, 0363, 0134, 0237, 0123, 0124, 0125, 0126,
    0127, 0130, 0131, 0132, 0364, 0365, 0366, 0367, 0370, 0371, 0060, 0061, 0062, 0063, 0064, 0065, 0066, 0067, 0070,
    0071, 0372, 0373, 0374, 0375, 0376, 0377};
...
int main(int argc, char *argv[]) {
    ...
    if (cols < 1 || ((hextype == HEX_NORMAL || hextype == HEX_BITS || hextype == HEX_LITTLEENDIAN) && (cols > COLS))) {
    ...
    if (revert) {
        ...
        return huntype(fp, fpo, stderr, cols, hextype, negseek ? -seekoff : seekoff);
    }
    ...
    if (hextype == HEX_CINCLUDE) {
        ...
        while ((length < 0 || p < length) && (c = getc(fp)) != EOF) {
            if (fprintf(fpo, (hexx == hexxa) ? "%s0x%02x" : "%s0X%02X", (p % cols) ? ", " : &",\n  "[2 * !p], c) < 0)
                die(3);
            p++;
        }
        if (c == EOF && ferror(fp))
    ...
    if (hextype == HEX_POSTSCRIPT) {
        ...
        while ((length < 0 || n < length) && (e = getc(fp)) != EOF) {
            if (putc(hexx[(e >> 4) & 0xf], fpo) == EOF || putc(hexx[e & 0xf], fpo) == EOF)
                die(3);
            n++;
            if (!--p) {
                if (putc('\n', fpo) == EOF)
    ...
    while ((length < 0 || n < length) && (e = getc(fp)) != EOF) {
        if (p == 0) {
            addrlen = sprintf(l, "%08lx:", ((unsigned long) (n + seekoff + displayoff)));
